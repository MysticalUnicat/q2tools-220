...
#define DOUBLEVEC_T // jit - might as well be more accurate, and sometimes doubles are even faster on modern hardware,
                    // anyway...
#endif
...
#define CrossProduct(v1, v2, cross)                                                                                    \
  ((cross)[0] = (v1)[1] * (v2)[2] - (v1)[2] * (v2)[1], (cross)[1] = (v1)[2] * (v2)[0] - (v1)[0] * (v2)[2],             \
...
#define VectorLengthSq(v) (v[0] * v[0] + v[1] * v[1] + v[2] * v[2])

#define VectorDistanceSq(x, y)                                                                                         \
...
qboolean RayPlaneIntersect(vec3_t p_n, vec_t p_d, vec3_t l_o, vec3_t l_n, vec3_t res);
...
static inline vec_t _DotProduct(vec3_t v1, vec3_t v2) { return v1[0] * v2[0] + v1[1] * v2[1] + v1[2] * v2[2]; }
...
static inline qboolean RayPlaneIntersect(vec3_t p_n, vec_t p_d, vec3_t l_o, vec3_t l_n, vec3_t res) {
  float dot, t;
