...
qboolean GetIntertexnormal(int32_t facenum1, int32_t facenum2) {
  ...
  if(!VectorNormalize(normal, normal) || DotProduct(normal, p1->normal) <= NORMAL_EPSILON ||
    ...
    for(k = 0; k < numfaces; k++) {
      ...
      for(i = 0; i < s->numedges; i++) {
        ...
        for(j = 0; j < 2; j++) // calculate st_mins, st_maxs
        {
          ...
          const vec_t val = (long double)v->point[0] * tex->vecs[j][0] + (long double)v->point[1] * tex->vecs[j][1] +
                            (long double)v->point[2] * tex->vecs[j][2] + tex->vecs[j][3];
          if(val < st_mins[j]) {
            st_mins[j] = val;
          }
          if(val > st_maxs[j]) {
...
int32_t AddFaceForVertexNormalX(const int32_t edgeabs, int32_t edgeabsnext, const int32_t edgeend, int32_t edgeendnext,
                                dface_tx *const f, dface_tx *fnext, vec_t angle, vec3_t normal)
...
int32_t AddFaceForVertexNormal(const int32_t edgeabs, int32_t edgeabsnext, const int32_t edgeend, int32_t edgeendnext,
                               dface_t *const f, dface_t *fnext, vec_t angle, vec3_t normal) {
    ...
    for(edgeabs = 0; edgeabs < mme; edgeabs++) {
        ...
        if(dedgesX[edgeabs].v[0] == dedgesX[edgeabs].v[1]) {
          ...
          for(edgeend = 0; edgeend < 2; edgeend++) {
            ...
            for(d = 0; d < 2; d++) {
              ...
              while(1) {
                fcurrent = fnext;
                r = AddFaceForVertexNormalX(edgeabsnext, edgeabsnext, edgeendnext, edgeendnext, fcurrent, fnext, angle,
                                            normal);
                count++;
                if(r == -1) {
                ...
                if(DotProduct(edgenormal, normal) + NORMAL_EPSILON < smoothing_threshold)
                  break;
                if(!GetIntertexnormal(fcurrent - dfacesX, e->facesX[0] - dfacesX) ||
                   !GetIntertexnormal(fcurrent - dfacesX, e->facesX[1] - dfacesX))
                  break;
                angles += angle;
        ...
        if(dedges[edgeabs].v[0] == dedges[edgeabs].v[1]) {
          ...
          for(edgeend = 0; edgeend < 2; edgeend++) {
            ...
            for(d = 0; d < 2; d++) {
              ...
              while(1) {
                fcurrent = fnext;
                r = AddFaceForVertexNormal(edgeabsnext, edgeabsnext, edgeendnext, edgeendnext, fcurrent, fnext, angle,
                                           normal);
                count++;
                if(r == -1) {
                  // qprintf("PairEdges: face edges mislink at (%f,%f,%f)", errorpos[0], errorpos[1], errorpos[2]);
                ...
                if(DotProduct(edgenormal, normal) + NORMAL_EPSILON < smoothing_threshold)
                  break;
                if(!GetIntertexnormal(fcurrent - dfaces, e->faces[0] - dfaces) ||
                   !GetIntertexnormal(fcurrent - dfaces, e->faces[1] - dfaces))
                  break;
                angles += angle;
                VectorMA(normals, angle, normal, normals);
                if(r != 0 || fnext == f)
      ...
      if(e->coplanar) {
        if(!VectorCompare(e->vertex_normal[0], e->interface_normal) ||
           !VectorCompare(e->vertex_normal[1], e->interface_normal)) {
          e->coplanar = false;
        }
      }
    }
...
===============
*/
void FreeTriangulation(triangulation_t *tr) { free(tr); }
...
void CalcFaceExtents(lightinfo_t *l) {
  ...
  if(l->texsize[0] * l->texsize[1] > map / 4) // div 4 for extrasamples
  {
    ...
    for(i = 0; i < 2; i++) {
      ...
      printf("  Mins = %10.3f, Maxs = %10.3f,  Size = %10.3f\n", (double)mins[i], (double)maxs[i],
             (double)(maxs[i] - mins[i]));
    }
...
void CreateDirectLights(void) {
  ...
  for(i = 0, p = patches; i < num_patches; i++, p++) {
    if((!sun || !p->sky) && p->totallight[0] < DIRECT_LIGHT && p->totallight[1] < DIRECT_LIGHT &&
...
static struct SH1 LightContributionToPoint(directlight_t *l, vec3_t pos, int32_t nodenum,
  ...
  float total_scale = lightscale2 * 0.125;
 ...
 */
static qboolean NudgeSamplePosition(const vec3_t in, const vec3_t normal, const vec3_t center, vec3_t out, byte *pvs) {
  vec3_t dir;
...
void BuildFacelights(int32_t facenum) {
  ...
  for(i = 0; i < liteinfo[0].numsurfpt; i++) {
    ...
    for(j = 0; j < numsamples; j++) {
      ...
      GatherSampleLight(pos, pointnormal, styletable, i * 3, tablesize, 1.0 / numsamples, &sun_main_once,
                        &sun_ambient_once, pvs);
    }
  ...
  if(use_qbsp) {
    ...
    if(numbounce > 0) {
      ...
      for(i = 0; i < f->numedges; i++) {
        ...
        if(ednum >= 0)
          AddPointToBounds(dvertexes[dedgesX[ednum].v[0]].point, facemins, facemaxs);
        else
          AddPointToBounds(dvertexes[dedgesX[-ednum].v[1]].point, facemins, facemaxs);
      }
